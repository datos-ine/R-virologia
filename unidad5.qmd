...
[Descargar este documento en PDF](unidad5.pdf){.btn .btn-primary}
...
Un ejemplo de ello, son todas las funciones de R base para comparaciones (pruebas de hipotesis). Tomemos el caso de la prueba **t de Student**, que sirve para comparar las medias de muestras aproximadamente *normales*.
...
Lo importante acá no es el resultado sino la forma en que se dá el input y output. Observaran que no se trata de un formato ordenado ni se parece a una tabla. El tidyverse siempre (salvo raras excepciones, como con `pull()`) devuelve una tabla de datos ordenada y por no cumplir con esa salida todas estas funciones son incompatibles, aún utilizando un `summarise()` (nos dan error):
...
Las funciones de inferencia estadística, para comparar medias y proporciones (métodos paramétricos y no paramétricos), ANOVAS, analisis post-hoc, correlaciones y tamaños de efecto, así como también valores p ajustados o agregados de etiquetas de significación se pueden ver en la documentación oficial del paquete:   <https://rpkgs.datanovia.com/rstatix/index.html>. Algunas de ellas las veremos en la próxima unidad sobre inferencia estadística.
...
Dentro de los estadísticos descriptivos del paquete, la función `get_summary_stats()` devuelve un resumen univariado para variables cuantitativas.
